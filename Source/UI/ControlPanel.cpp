...
#ifdef _WIN32
	m_socket_listener = new ButtonSocketListener(this);
#else
...
void RecordButton::StartRecording()
{
	MessageManagerLock mml;
	this->setToggleState(true, sendNotificationSync);
}

void RecordButton::StopRecording()
{
	MessageManagerLock mml;
	this->setToggleState(false, sendNotificationSync);
}
...

{
    ...
    if (button == recordButton)
    {
        if (recordButton->getToggleState())
        {
			OutputDebugString("recordButton: StartRecording\n");
        ...
        {
			OutputDebugString("recordButton: StopRecording\n");
            stopRecording();
        }
    }
